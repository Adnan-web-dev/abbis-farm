        ...
        <div class="table-responsive">
            <table id="transaction_show-table" class="table ">
                <thead>
                    ...
                    <tr>
                        <?php if(config('variable.currency_format') == 'suffix'): ?>
                            <th colspan="7" class="text-center"><?php echo e(__('Initial Balance')); ?>

                                (<?php echo e(config('variable.currency')); ?>
                ...
                <tbody>
                    <?php $__currentLoopData = $transactions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $transaction): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <tr data-id="<?php echo e($transaction->id); ?>">
                            ...
                            <td><?php echo e($transaction->expense_reference ? $transaction->expense_date : $transaction->deposit_date); ?>
                            ...
                            <?php else: ?>
                                <td><?php echo e($transaction->expense_reference ? trans('file.Expense') : trans('file.Income')); ?>
                ...
                var table_table = $('#transaction_show-table').DataTable({
                    ...
                    buttons: [{
                            ...
                            customize: function(doc) {
                                ...
                                var creditTotal = 0;
                                var debitTotal = 0;
                                var tableData = table_table.rows({
                                    filter: 'applied'
                                }).data().toArray();

                                for (var i = 0; i < tableData.length; i++) {
                                    var creditAmount = parseFloat(tableData[i][3].replace(
                                        /[\$,]/g, '')) || 0;
                                    var debitAmount = parseFloat(tableData[i][4].replace(
                                        /[\$,]/g, '')) || 0;
                                    creditTotal += creditAmount;
                                    debitTotal += debitAmount;
                                }

                                var balanceTotal = parseFloat(bla) + creditTotal - debitTotal;

                                // Add total credit, debit, and balance to the PDF
                                doc.content.push({
                                    text: 'Total Credit: <?php echo e(config('variable.currency')); ?> ' +
                                ...
                                doc.content.push({
                                    text: 'Total Debit: <?php echo e(config('variable.currency')); ?> ' +
                                ...
                                doc.content.push({
                                    text: 'Balance: <?php echo e(config('variable.currency')); ?> ' +
...
<?php echo $__env->make('layout.main', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\laravel\abbisfarm\resources\views/finance/transaction/show.blade.php ENDPATH**/ ?>