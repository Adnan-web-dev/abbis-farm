                                        ...
                                        <div class="row">
                                            <div class="col-md-4">
                                                <div class="form-group">
                                                    ...
                                                    <select class="form-control selectpicker dynamic" name="filter_company"
                                                        id="company_id" data-first_name="first_name"
                                                        data-last_name="last_name" data-placeholder="Company"
                                                ...
                                                <div class="form-group">
                                                    <div class="form-actions">
                                                        <button type="submit" id="filterSubmit"
                                                            class="filtering btn btn-primary"><i
                        ...
                        buttons: [{
                                ...
                                customize: function(doc) {
                                    ...
                                    for (var i = 0; i < tableData.length; i++) {
                                        // Ensure salary_details is a string containing HTML
                                        var salaryDetails = tableData[i].salary_details;
                                        if (salaryDetails) {
                                            // Parse the salaryDetails HTML string
                                            var $salaryDetails = $('<span>').html(salaryDetails);
                                                var netPayableString = $salaryDetails.find(
                                                    '.net_payable').text();
                                                    var netPayable = parseFloat(netPayableString
                                                    .replace(/[\$,]/g, ''));
                                                    console.log(netPayable);
                                            totalNetPayable += isNaN(netPayable) ? 0 :
                                                netPayable;
                                        }
                                    }

                                    // Format the total net payable with currency
                                    var formattedTotal = '<?php echo e(config('variable.currency')); ?> ' +
                                        totalNetPayable.toFixed(2);

                                    // Add total net payable to the PDF
                                    doc.content.push({
                                        text: 'Total Paid Salary: ' + formattedTotal,
                                        alignment: 'left',
                                        margin: [0, 10, 0,
                ...
                if ($(this).val() !== '') {
                    ...
                    $.ajax({
                        ...
                        success: function(result) {
                            ...
                            $('.default_emp').prepend(
                                '<option value="0" selected><?php echo e(__('All Employees')); ?></option>'
                                );
                            $('select').selectpicker();
...
<?php echo $__env->make('layout.main', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\laravel\abbisfarm\resources\views/salary/payslip/index.blade.php ENDPATH**/ ?>