            ...
            <ul id="side-main-menu" class="side-menu list-unstyled">
                ...
                @if (Auth::user()->role_users_id===1)
                    <li class="{{ (request()->is('addons*')) ? 'active' : '' }}">
                        <a href="{{route('addons')}}"> <i class="dripicons-ticket"></i><span>{{__('Abbis Farm Procurement')}}</span>
                        </a>
                    </li>
                @endif
                    ...
                    <li class="has-dropdown @if(request()->is('user*')){{ (request()->is('user*')) ? 'active' : '' }}@elseif(request()->is('add-user*')){{ (request()->is('add-user*')) ? 'active' : '' }}@endif">
                        @if(auth()->user()->can('view-user'))
                            <a href="#users" aria-expanded="false" data-toggle="collapse">
                                ...
                                <span>{{trans('Abbis Farm Users')}}</span>
                            </a>
                        @endif
                ...
                @can('view-details-employee')
                    <li class="has-dropdown {{ (request()->is('staff*')) ? 'active' : '' }}">
                        <a href="#employees" aria-expanded="false" data-toggle="collapse"> <i class="dripicons-user-group"></i><span>{{trans('Abbis Farm Employees')}}</span></a>
                        <ul id="employees" class="collapse list-unstyled ">
                            @can('view-details-employee')
                                <li id="employee_list"><a href="{{route('employees.index')}}">{{__('Abbis Farm Employee Lists')}}</a></li>
                            @endcan
                            @can('import-employee')
                        ...
                        <ul id="Customize_settings" class="collapse list-unstyled ">
                            ...
                            @can('access-variable_type')
                                <li id="variable_type"><a
                                            href="{{route('variables.index')}}">{{__('Abbis Farm Procedures')}}</a>
                                </li>
                            @endcan
                            @can('access-variable_method')
                                <li id="variable_method"><a
                                            href="{{route('variables_method.index')}}">{{__('Abbis Farm Structures')}}</a>
                                </li>
                            @endcan
                            @can('view-general-setting')
                ...
                <li class="has-dropdown {{ (request()->is('organization*')) ? 'active' : '' }}"><a href="#Organization" aria-expanded="false" data-toggle="collapse">
                        <i
                                class="dripicons-view-thumb"></i><span>{{trans('Abbis Farm Organization')}}</span></a>
                    <ul id="Organization" class="collapse list-unstyled ">
                        @can('view-company')
                            <li id="company"><a href="{{route('companies.index')}}">{{trans('Abbis Farm Company')}}</a></li>
                        @endcan
                        @can('view-department')
                            <li id="department"><a
                                        href="{{route('departments.index')}}">{{trans('Abbis Farm Department')}}</a>
                            </li>
                        @endcan

                        @can('view-location')
                            <li id="location"><a href="{{route('locations.index')}}">{{trans('Abbis Farm Location')}}</a></li>
                        @endcan
                        @can('view-designation')
                            <li id="designation"><a
                                        href="{{route('designations.index')}}">{{trans('Abbis Farm Designation')}}</a>
                            </li>
                        @endcan

                        {{-- @can('announcement') --}}
                            <li id="announcements"><a href="{{route('announcements.index')}}">{{trans('Abbis Farm Announcements')}}</a></li>
                        {{-- @endcan --}}

                        {{-- @can('policy') --}}
                            <li id="company_policy"><a href="{{route('policy.index')}}">{{__('Abbis Farm Company Policy')}}</a></li>
                        {{-- @endcan --}}
                    </ul>
                </li>
                    ...
                    <li class="has-dropdown {{ (request()->is('payroll*')) ? 'active' : '' }}">
                        ...
                        @if(auth()->user()->can('view-payslip') || auth()->user()->can('view-paylist'))
                            <a href="#Payroll" aria-expanded="false" data-toggle="collapse">
                                <i class="dripicons-wallet"></i><span>{{trans('Pay Salary')}}</span>
                            </a>
                        @endif

                        <ul id="Payroll" class="collapse list-unstyled ">
                            @can('view-payslip')
                                <li><a href="{{route('payroll.index')}}">{{__('Pay New Salary')}}</a>
                                </li>
                            @endcan
                            @can('view-paylist')
                                <li><a href="{{route('payment_history.index')}}">{{__('Paid Salary History')}}</a>
                                </li>
                            @endcan
                        </ul>
                    </li>
                        ...
                        <li class="has-dropdown {{ (request()->is('accounting*')) ? 'active' : '' }}">
                            ...
                            @if(auth()->user()->can('view-account') || auth()->user()->can('view-payee') || auth()->user()->can('view-payer') ||auth()->user()->can('view-deposit')||auth()->user()->can('view-expense')||auth()->user()->can('view-transaction')||auth()->user()->can('view-balance_transfer'))
                                <a href="#Finance" aria-expanded="false" data-toggle="collapse">
                                    <i class="dripicons-graph-pie"></i><span>{{trans('Abbis Farm Finance')}}</span>
                                </a>
                            @endcan
                            ...
                            <ul id="Finance" class="collapse list-unstyled ">
                                @can('view-account')
                                    <li id="accounting_list"><a
                                                href="{{route('accounting_list.index')}}">{{__('Abbis Farm Bank Accounts List')}}</a>
                                    </li>
                                @endcan
                                @can('view-account')
                                    <li id="account_balances"><a
                                                href="{{route('account_balances')}}">{{__('Abbis Farm Bank Account Balances')}}</a>
                                    </li>
                                @endcan
                                @can('view-payee')
                                    <li id="payees"><a
                                ...
                                @can('view-deposit')
                                    <li id="deposit"><a
                                                href="{{route('deposit.index')}}">{{trans(('Abbis Farm Bank Deposit'))}}</a>
                                    </li>
                                @endcan
                                @can('view-expense')
                                    <li id="expense"><a
                                                href="{{route('expense.index')}}">{{trans(('Abbis Farm Expense'))}}</a>
                                    </li>
                                @endcan
                                @can('view-transaction')
                                    <li id="transactions"><a
                                                href="{{route('transactions.index')}}">{{trans(('Abbis Farm Bank Transaction'))}}</a>
                                    </li>
                                @endcan
                                @can('view-balance_transfer')
                                    <li id="finance_transfer"><a
                                                href="{{route('finance_transfer.index')}}">{{trans(('Abbis Farm Bank Transfer'))}}</a>
                                    </li>
                                @endcan
                            </ul>
                        </li>
