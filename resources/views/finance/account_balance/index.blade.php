  ...
  border-collapse: collapse;
        ...
        <div class="table-responsive">
            <table id="account_balance-table" class="table ">
                <thead>
                <tr>
                    <th>{{trans('Account(s)')}}</th>
                    @if(config('variable.currency_format')=='suffix')
                        <th>{{trans('Balance(s)')}} ({{config('variable.currency')}})</th>
                    @else
                        <th>({{config('variable.currency')}}) {{trans('Balance(s)')}}</th>
                    @endif
                </tr>
                </thead>
         ...
         let  table = $('#account_balance-table').DataTable({
             ...
             "footerCallback": function ( row, data, start, end, display ) {
                 ...
                 $( api.column( 1 ).footer() ).html(
                     +pageTotal +' ('+ total +' {{trans (':-  Total Balance on All Abbis Farm Accounts')}})'
                 );
             },
