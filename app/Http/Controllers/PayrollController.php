			...
			if (request()->ajax())
			{
				...
				if (!empty($request->filter_company && $request->filter_department))
				{
					$employees = Employee::with(['salaryBasic' => function ($query)
						...
						->leftJoin('employee_bank_accounts','employees.id','=','employee_bank_accounts.employee_id')
						->select('employee_bank_accounts.account_number','employee_bank_accounts.bank_name','employees.id', 'employees.first_name', 'employees.last_name', 'employees.basic_salary', 'employees.payslip_type','employees.pension_type','employees.pension_amount')
